...
var controller = {
    ...
    saveProject: function (req, res) {
        ...
        //guardar datos en documento/Proyecto
    ...
    //actualizar objetos del proyecto
    ...
    uploadImage(req, res) {
        ...
        if (req.files) {
            ...
            if (fileExt == 'png' || fileExt == 'jpg' || fileExt == 'jpeg' || fileExt == 'jfif' || fileExt == 'gif') {
                Project.findByIdAndUpdate(projectId, { image: fileName }, { new: true }, (err, projectUpdate) => {

                    if (err) return res.status(500).send({ message: 'La imagne no se ha subido.' })

                    if (!projectUpdate) return res.status(404).send({ message: 'El proyecto no existe y no se sube la imagen' })

                    return res.status(200).send({ files: projectUpdate })
                })
            }else{
